...
class EstadoScrapy(scrapy.Spider):
    name = "PI"
    start_urls = [
        'http://www.diariooficial.pi.gov.br/diarios.php?dia=20190125']
    ...
    def parse(self, response):
        ...
        next_page = response.xpath(
            '//table[@width="350px"]/tr[3]//td[@align="left"]//a[@class="texto2"]/@href').extract()
        for url in next_page:
            yield scrapy.Request(url=urljoin(self.url_base, url), callback=self.parse)

    def download_pdfs(self, response):
        name_file = response.xpath(
            '//td[@class="titulo"]//text()').extract_first().strip()
        path = name_file.split(' ')[-1].split('.')[-1]
        url_pdf = response.xpath(
            '//span[@class="texto_diario2"]//@href').extract()
        for url in url_pdf:
